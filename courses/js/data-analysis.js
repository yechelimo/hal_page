    ...
    setTimeout(() => {
        if (code.includes('print(')) {
            ...
            if (printMatch) {
                ...
                outputArea.innerHTML = '<span class="text-gray-300">代码执行成功，但没有输出</span>';
            }
        } else if (code.includes('data.describe()')) {
            outputArea.innerHTML = `<pre class="whitespace-pre-wrap">          A          B          C
count  100.000000  100.000000  100.000000
mean   100.736907   50.095697   20.038152
std     9.423629    4.706434    1.970535
min    77.030262   37.810209   14.938403
25%    94.254010   47.110867   18.629202
50%   100.342649   49.906916   20.056051
75%   106.997353   52.948491   21.362947
max   125.586604   62.893359   25.134931</pre>`;
        } else if (code.includes('plt.show()')) {
            outputArea.innerHTML = '<span class="text-gray-300">图表已显示</span>';
        } else if (code.includes('t_stat, p_value')) {
            outputArea.innerHTML = `<pre class="whitespace-pre-wrap">t统计量: -3.6288
p值: 0.0004
两个样本的均值有显著差异</pre>`;
        } else if (code.includes('correlation_matrix')) {
            outputArea.innerHTML = `<pre class="whitespace-pre-wrap">相关系数矩阵:
          x         y         z
x  1.000000  0.943662  0.044766
y  0.943662  1.000000  0.022155
z  0.044766  0.022155  1.000000</pre>`;
        } else if (code.includes('model.coef_')) {
            outputArea.innerHTML = `<pre class="whitespace-pre-wrap">斜率: 2.0176
截距: 0.8093</pre>`;
        } else if (code.includes('accuracy')) {
            outputArea.innerHTML = `<pre class="whitespace-pre-wrap">模型准确率: 1.0000

测试集预测结果:
[1 0 2 1 1 0 1 2 1 1 2 0 0 0 0 1 2 1 1 2 0 2 0 2 2 2 2 2 0 0]

测试集实际结果:
[1 0 2 1 1 0 1 2 1 1 2 0 0 0 0 1 2 1 1 2 0 2 0 2 2 2 2 2 0 0]</pre>`;
        } else {
...
function submitAnswer() {
    ...
    if (problem.type === 'code') {
        // 简单的代码答案检查
        if (codeEditor.value.includes('print("Hello, World!")') || 
            codeEditor.value.includes('data.dropna()') ||
            codeEditor.value.includes('StandardScaler()') ||
            codeEditor.value.includes('SelectKBest') ||
            codeEditor.value.includes('plt.plot') ||
            codeEditor.value.includes('sns.barplot') ||
            codeEditor.value.includes('sns.heatmap') ||
            codeEditor.value.includes('data.describe()') ||
            codeEditor.value.includes('stats.ttest_ind') ||
            codeEditor.value.includes('data.corr()') ||
            codeEditor.value.includes('LinearRegression()') ||
            codeEditor.value.includes('DecisionTreeClassifier()') ||
            codeEditor.value.includes('KMeans()')) {
